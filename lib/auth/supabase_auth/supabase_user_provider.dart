...
class QRPresencaSupabaseUser extends BaseAuthUser {
  QRPresencaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> qRPresencaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = QRPresencaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
